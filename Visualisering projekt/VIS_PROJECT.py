...
from plotly.graph_objects import Figure, go
import numpy as np



# These paths need to be updated before running the code
geojson_path = "//output_file.geojson"
csv_file_path = r"//ElectricCarData_Clean.csv"
file_path = ('/VISP/')
...
trip_times = [30, 60]
# Load network map
G = ox.load_graphml((file_path + 'denmark_driving_network_from_place.graphml'))
...
def get_city(name,file_path):
    pd_df = pd.read_csv((file_path + 'Danmarks_Storste_Byer.csv'), sep=",")
...
def check_map(selection_data, dropdown_value, travel_speed=50):
    ...
    file_path = f"/Users/oliverbendorf/Desktop/VISP/{file_name}"
